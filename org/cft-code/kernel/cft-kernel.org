:PROPERTIES:
:ID:       20260606T150002.000000
:END:
#+title: CFT Kernel
#+auto_tangle: t
#+PROPERTY: header-args:zig :tangle ../../../src/cft-code/kernel.zig :mkdirp yes

The kernel is the runtime executor for a CFT preset.  It does not define what a
CFT is; that belongs to [[id:20260605T090102.250727][Theory]].  The kernel receives a theory declaration at
=comptime=, builds a generated type from it, and exposes only the small
runtime operations that [[id:20260603T094138.285453][Correlators]], [[id:20260603T094112.928043][OPE]], and [[id:20260606T063253.759714][Basis-Generation]] need.
...
A [[id:20260603T094138.285453][correlator]] or [[id:20260603T094112.928043][OPE]] should emit terms as soon as possible instead of
materializing a large [[id:20260605T082209.767536][Expression]].  The stream namespace holds the small output
records used by sinks.
...
The public [[id:20260603T094112.928043][OPE]] and [[id:20260603T094138.285453][correlator]] accept any number of local [[id:20260603T094125.997360][Operators]] through a
=MultiOp=.  At runtime a local operator is only its insertion, operator kind,
and a span into an immutable label store.  The typed preset builder will create
these spans; the kernel only needs compact data for fast Wick and zero-mode
passes.

Projection and [[id:20260606T063253.759714][basis]] queries are call data; [[id:20260605T090102.250727][Theory]] owns the
quantum-number, weight, domain, and mode ids they refer to.
...
The body store is generated from a theory operator schema.  Later
implementations will validate labels and canonicalize normal products here;
the first version only build-checks the boundary.
...
The store is a generated type because its schema is preset data.  This is the
right use of =comptime=: it specializes a hot construction boundary without
moving runtime operator labels or generated basis states to compile time.

* Generated Kernels

The generated bulk kernel owns no boundary data.  It receives a bulk
=Spec.CFT=, exposes public runtime operations, and keeps preset tables private
...
The BCFT generated kernel extends an already generated bulk kernel.  Boundary
data enters only through the =Spec.BCFT= supplied here.
...
The first test protects the runtime call shape: a =MultiOp= carries the local
operators and the label store needed to interpret them, without receiving a
separate context.
...
The second test checks the kernel/theory boundary that matters now: a BCFT
generated kernel extends the body schema without mutating the bulk generated
kernel.
