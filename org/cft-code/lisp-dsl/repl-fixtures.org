:PROPERTIES:
:ID:       20260608T235500.000000
:END:
#+title: Lisp Generated Fixture Declarations
#+auto_tangle: t
#+PROPERTY: header-args:lisp :tangle ../../../lisp/string-code-cft-fixtures.lisp :mkdirp yes

This node records the minimal Lisp-owned declarations for the generated
fixtures.  They use the generic emitter from
[[id:20260608T234500.000000][Lisp Descriptor Emitter]] and add one REPL macro
that owns runtime lifetime and returns the symbolic expression directly.
...
The REPL macro layer rewrites user field forms into explicit runtime insertions.
=ops= just groups forms; =R= groups forms and then asks Zig to normal-order the
number of fields in that group.  No symbolic expression is built before the
operator list is frozen.
...
Preset accessors, runtime constructors, and field inserters are generated from
the preset compiler metadata.  Public aliases such as =b-field= and =c-field=
remain stable, but field ids and insertion arities come from the descriptor.
