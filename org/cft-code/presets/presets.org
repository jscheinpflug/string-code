...
Only handle construction and scalar convention helpers cross from the shared
runtime into the root public API.
...
re-export these family objects without proliferating flat constructor and
config names.
