:PROPERTIES:
:ID:       20260614T235500.000000
:END:
#+title: Tensor Public Audit Benchmark
#+auto_tangle: t
#+PROPERTY: header-args:zig :tangle ../../benchmarks/tensor_public_audit.zig :mkdirp yes

This explicit audit exercises public tensor-code routes whose full basis
enumeration is too large for the default unit tests.  The SO(10) hook-six row
checks public formula coverage, verifies a primitive tensor-only page, and
proves that a zero-term public render budget fails by policy rather than by a
named-projector fallback.
