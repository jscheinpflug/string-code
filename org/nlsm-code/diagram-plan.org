:PROPERTIES:
:ID:       20260609T212500.000000
:END:
#+title: NLSM Diagram Plan
#+auto_tangle: t
#+PROPERTY: header-args:zig :tangle ../../src/nlsm-code/diagram_plan.zig :mkdirp yes

This note implements the first explicit diagram-planning layer on top of the
[[id:20260609T210500.000000][NLSM RNC Vertices]] rows.  The current module enumerates compact vertex-multiset
candidates for a requested loop order and local operator signature.  It does
not yet build full connected Wick graphs.  That is deliberate: we need a
stable, inspectable row format before adding the more expensive pairing stage.

The row shape is target-agnostic.  CY3 and K3 later change the target-flavor
quotient and the vertex catalog, but not the candidate-plan format.
