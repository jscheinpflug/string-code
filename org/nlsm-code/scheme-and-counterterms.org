:PROPERTIES:
:ID:       20260609T201500.000000
:END:
#+title: NLSM Scheme And Counterterms
#+auto_tangle: t
#+PROPERTY: header-args:zig :tangle ../../src/nlsm-code/scheme.zig :mkdirp yes

This note fixes the first concrete runtime layer behind the
[[id:20260609T193000.000000][Type II R4 Implementation Plan]]: the stringbook-style dimensional-regularization
scheme descriptor and the streamed pole-row data structures.  The point is to
record exactly what the contraction and pole-extraction stages must emit, while
keeping target geometry as external row data so the same layer works for real
Riemannian, Kahler, CY3, and K3 quotients.

* Scheme

The regulator is data.  The contraction kernels only need to know that a job
is using the stringbook-compatible \(d=2-\epsilon\) minimal-subtraction setup.

#+begin_src zig :tangle ../../src/nlsm-code/scheme.zig
...
The counterterm rows sit next to the scheme because they are the first stable
boundary between contraction and renormalization.  The rows are generic in the
target presentation: the geometry flavor and local tensor word are attached as
opaque streamed data.

#+begin_src zig :tangle ../../src/nlsm-code/counterterms.zig
const std = @import("std");
