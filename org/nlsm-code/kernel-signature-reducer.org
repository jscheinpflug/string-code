...
This note is the bridge from local Wick data to regulated master families.  The
input is still local: vertex multiplicities, explicit pairings, and compact
target tensors.  The output is also local: one =KernelTermRow= with a reduced
kernel signature and one tensor word.

The physics boundary is strict.  This module may:

- multiply exact rational symmetry factors;
- accumulate the target tensors attached to the chosen RNC vertices;
- translate the differentiated propagators of one branch into a compact kernel
  descriptor.

It must not:

- evaluate the loop integral;
- simplify geometry identities;
- infer beta functions.

The new point in this revision is that the branch-driven path no longer guesses
the numerator structure from species counts alone.  Each explicit pair now
brings a =PairKernelFactor=.  The reducer sums those local differentiated
propagator factors into the kernel signature that the pole extractor matches.

For bosons, the primitive pair kind already fixes the derivative structure.
For fermions, the pair kind stores only the chirality, while the total number
of chiral derivatives on that pair is stored explicitly as data.  This avoids
turning \(\partial^n\psi\,\partial^m\psi\) into an ever-growing enum at the
API boundary.  The reduced kernel signature therefore keeps:

- a compact histogram of primitive pair kinds;
- the total derivative order carried by all left-moving fermion pairs;
- the total derivative order carried by all right-moving fermion pairs.

The next refinement is also local and explicit: the reduced signature now
stores the vertex count, pair count, and the separate surviving background-leg
counts \((\partial X_0,\bar\partial X_0)\).  This is the minimum extra data
needed to distinguish the first genuinely supersymmetric mixed branch already
present in the component source table:

\[
\big(\bar\partial X_0\,\xi\,\psi_L\psi_L\,R\big)
\times
\big(\partial X_0\,\xi\,\psi_R\psi_R\,R\big).
\]

After Wick contraction this branch has one bosonic pair, one left-fermion
pair, one right-fermion pair, two surviving background legs, and total
combinatorial weight \(({-1/3})^2 = 1/9\).  The reducer should preserve that
structure exactly rather than collapse it into the old bosonic low-loop
placeholder.
