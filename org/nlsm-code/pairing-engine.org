:PROPERTIES:
:ID:       20260609T224500.000000
:END:
#+title: NLSM Pairing Engine
#+auto_tangle: t
#+PROPERTY: header-args:zig :tangle ../../src/nlsm-code/pairing_engine.zig :mkdirp yes

This note replaces the species-count Wick summary by an explicit pairing stage
fed by the actual =cft-code= correlator for the current bootstrap worldsheet
models.  Each admissible branch says two things:

- which quantum field occurrences were paired;
- which primitive free-field kernel each pair carries.

The second point is constrained by [[id:20260609T233500.000000][Primitive Pair
Kernels]] in =cft-code=.  The =nlsm-code= build now depends on =cft-code= at
the package boundary, so the primitive free-field inventory is single-sourced
again.  This matters physically.  The free theory does not contain an
independent primitive \(\langle \partial \xi
\,\bar\partial \xi \rangle\) Wick rule in the current sphere preset inventory.
A branch that would require such a primitive pair is therefore dropped here
rather than relabelled as a new local propagator family.

That keeps the separation clean:

- =cft-code= owns the primitive free-field two-point rule inventory;
- =nlsm-code= owns branch enumeration for RNC vertices.

The same separation now applies to differentiated fermion pairs.  The pairing
engine does not invent a special NLSM-only \(\partial\psi\,\psi\) rule.  It
asks =cft-code= for the chiral derivative profile of the free-fermion pair and
receives back the corresponding derived kernel kind.

There is one physics point that must be fixed here.  The RNC interaction
vertices are lowered as local insertions at one worldsheet point, /without/
normal-order protection.  If they were normal ordered, the local
\(\xi\xi\) self-contractions inside a single interaction vertex would be
removed, but those local contractions are part of the counterterm already at
one loop.  Normal ordering is a statement about composite operators.  The
background-field interaction vertex is part of the action, so the correlator
must see its same-point contractions.

The same row format remains suitable for later CY3/K3 work because the pairing
engine still knows only quantum species, worldsheet derivatives, and branch
signs.  It does not know target-space cohomology or K\"ahler quotients.
