...
:END:
#+title: NLSM Pole Extractor
#+auto_tangle: t
#+PROPERTY: header-args:zig :tangle ../../src/nlsm-code/pole_extractor.zig :mkdirp yes

This note implements the first explicit bridge from reduced loop-kernel
signatures to local pole rows.  The extractor does not know what target
geometry means.  It only carries the target flavor and the local tensor term
through unchanged, so the same machinery works for the ordinary real-RNC
type-II calculation and later CY3/K3 specializations.

The rule table is intentionally explicit.  The contraction stage emits compact
=KernelSignature= rows.  The pole extractor matches those signatures against a
small analytic table derived in the note and emits the local
[[id:20260609T201500.000000][NLSM Scheme And Counterterms]] rows needed by beta assembly.
