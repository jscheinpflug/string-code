...
pub const RenderOptions = struct {
    ...
    lower_generalized_delta_atoms: bool = false,
    gamma_only: bool = false,
    first_term: usize = 0,
    max_terms: ?usize = null,
};
...
/// rationalFromWide normalizes an exact wide rational coefficient.
pub fn rationalFromWide(numerator: i128, denominator: u128) !RationalId {
    if (denominator == 0) return error.InvalidRationalDenominator;
    if (numerator == 0) return .{ .numerator = 0, .denominator = 1 };
    const divisor = gcdU128(absI128(numerator), denominator);
    return .{
        .numerator = @divExact(numerator, @as(i128, @intCast(divisor))),
        .denominator = @divExact(denominator, divisor),
    };
}
