...
fn lowerPrimaryInfinity(comptime d: Descriptor, comptime field: Field) ?declare.Spec.InfinityBehavior {
    ...
    return .{ .primary = .{
        .holomorphic_power = holomorphic_power,
        .antiholomorphic_power = antiholomorphic_power,
    } };
}
...
fn lowerField(comptime d: Descriptor, comptime field: Field) declare.Spec.Operator {
    ...
    return .{
        ...
        .infinity = lowerInfinity(d, field),
    };
}
...
const EventSink = struct {
    ...
    pub fn emitWickCoordinate(self: *@This(), factor: anytype) !void {
        switch (factor.kernel) {
            ...
            .point_power => |item| try self.emit(.{ .kind = .coordinate, .a = item.point, .d = item.exponent, .name = "point-power" }),
...
fn insertionFrom(shape: InsertionShape, coords: []const u32) !operators.OperatorInsertion {
    return switch (shape) {
        .single => if (coords.len == 1) .{ .single = .{ .position = pointFromCoord(coords[0]), .derivatives = 0 } } else error.InvalidCoordinateArity,
        .pair => if (coords.len == 2) .{ .pair = .{
            .holomorphic_position = pointFromCoord(coords[0]),
            .antiholomorphic_position = pointFromCoord(coords[1]),
...
fn pointFromCoord(z: u32) operators.Point {
    return if (z == operators.Point.infinity_variable) operators.Point.infinity() else operators.Point.finite(z);
}

fn singleInsertion(z: u32, derivatives: u8) operators.OperatorInsertion {
    return .{ .single = .{ .position = pointFromCoord(z), .derivatives = derivatives } };
}

fn pairInsertion(z: u32, zbar: u32) operators.OperatorInsertion {
    return .{ .pair = .{
        .holomorphic_position = pointFromCoord(z),
        .antiholomorphic_position = pointFromCoord(zbar),
        .holomorphic_derivatives = 0,
        .antiholomorphic_derivatives = 0,
    } };
}
...
pub fn GeneratedTheory(comptime d: Descriptor) type {
    ...
    const fields = fieldSpecStorage(d);
    const infinity_data = declare.Spec.infinityData(&fields);
    ...
    return struct {
        ...
        pub const Context = struct {
            ...
            /// infinity returns the raw coordinate token for insertion at infinity.
            pub fn infinity(_: *Context) u32 {
                return operators.Point.infinity_variable;
            }
...
test "generated theory exposes field builders and direct correlator sink" {
    ...
    const fields = [_]Field{
        .{ .id = 0, .symbol = 3, .insertion = .single, .labels = &.{.{ .id = 0, .role = .vector_index, .symbol = 4 }}, .statistics = .fermionic, .weight = 0 },
    };
    ...
    var infinity_ctx = theory.local(std.testing.allocator);
    defer infinity_ctx.deinit();
    const finite = try infinity_ctx.coord("finite");
    const infinity = infinity_ctx.infinity();
    const alpha = try infinity_ctx.index("alpha");
    const beta = try infinity_ctx.index("beta");
    try Psi.single(&infinity_ctx, finite, 0, .{alpha});
    try Psi.single(&infinity_ctx, infinity, 0, .{beta});
    const infinity_ops = try infinity_ctx.ops();

    var infinity_sink = Sink{};
    try theory.correlator(&theory.config.sphere, infinity_ops, &infinity_sink);

    try std.testing.expectEqual(@as(usize, 1), infinity_sink.wick_terms);
    try std.testing.expectEqual(@as(usize, 0), infinity_sink.coordinates);
    try std.testing.expectEqual(@as(usize, 1), infinity_sink.tensors);
    try std.testing.expectEqual(@as(usize, 1), infinity_sink.base_cases);
}
