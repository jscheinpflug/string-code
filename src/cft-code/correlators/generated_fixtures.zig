...
const free_boson_symbols = [_][]const u8{ "free-boson-10", "alpha-prime", "spin10", "d5", "dX", "mu", "vector", "eta", "expX", "k" };
...
const free_boson_fields = [_]d.Field{
    .{ .id = 0, .symbol = 4, .insertion = .single, .labels = &.{.{ .id = 0, .role = .vector_index, .symbol = 5 }}, .statistics = .bosonic },
    .{ .id = 1, .symbol = 8, .insertion = .pair, .labels = &.{.{ .id = 0, .role = .momentum, .symbol = 9 }}, .statistics = .bosonic, .zero_mode_consumable = true, .weight = 5, .anti_weight = 5 },
};
const free_boson_metadata = [_]d.MetadataExpr{
    .{ .rational = .{ .numerator = 1, .denominator = 4 } },
    .{ .parameter = 0 },
    .{ .field_label = .{ .field = 1, .label = 0 } },
    .{ .bilinear = .{ .left = 2, .right = 2, .form_symbol = 7 } },
    .{ .mul = .{ .left = 1, .right = 3 } },
    .{ .mul = .{ .left = 0, .right = 4 } },
};
...
const free_boson_wick = [_]d.WickRule{
    .{ .surface = 0, .left = 0, .right = 0, .terms = &free_boson_terms_0 },
    .{ .surface = 0, .left = 0, .right = 1, .terms = &free_boson_terms_1 },
    .{ .surface = 0, .left = 1, .right = 1, .terms = &free_boson_terms_2 },
};
const free_boson_zero_modes = [_]d.ZeroModeRule{
    .{ .surface = 0, .kind = .boson_momentum_conservation, .consumes = &.{1}, .two_pi_power = 10 },
};
