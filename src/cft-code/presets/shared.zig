...
fn emitPrimaryInfinityCoordinate(factor: Correlator.ResolvedCoordinateFactor, branch: anytype) !void {
    switch (factor) {
        .difference_power => |item| {
            ...
            branch.infinity_branch_power += -@as(i32, item.exponent);
            if (left_inf and !right_inf and oddIntegerPower(item.exponent)) {
                try branch.emitWickScalar(Correlator.ResolvedScalarFactor{ .value = scalars.rational(-1, 1) });
            }
        },
        .logarithm => |item| {
...
fn emitPrimaryInfinityWickTerm(pair: Correlator.WickPair, left_index: usize, right_index: usize, term_index: usize, branch: anytype) !void {
    const resolved = (Correlator.ResolvedWickPair{ .pair = pair }).term(term_index) orelse return error.InvalidWickTerm;
    ...
    var scalar_index: usize = 0;
    while (scalar_index < resolved.scalarCount()) : (scalar_index += 1) {
        try branch.emitWickScalar(resolved.scalar(scalar_index) orelse return error.InvalidWickFactor);
    }

    var coordinate_index: usize = 0;
    while (coordinate_index < resolved.coordinateCount()) : (coordinate_index += 1) {
        try emitPrimaryInfinityCoordinate(resolved.coordinate(coordinate_index) orelse return error.InvalidWickFactor, branch);
    }

    var tensor_index: usize = 0;
    while (tensor_index < resolved.tensorCount()) : (tensor_index += 1) {
        try branch.emitWickTensor(resolved.tensor(tensor_index) orelse return error.InvalidWickFactor);
    }

    var action_index: usize = 0;
    while (action_index < resolved.actionCount()) : (action_index += 1) {
        try branch.emitWickAction(resolved.action(action_index) orelse return error.InvalidWickFactor);
    }

    var residual_index: usize = 0;
    while (residual_index < resolved.residualCount()) : (residual_index += 1) {
        try emitWickResidualOperator(pair, left_index, right_index, resolved.residual(residual_index) orelse return error.InvalidWickFactor, branch);
    }
...
fn AccumulatedBranch(comptime ConfigHandle: type, comptime mode: WickMode) type {
    return struct {
        ...
        fn ensurePrimaryInfinity(self: *Self, ops: kernel.Call.MultiOp) !void {
            ...
            const info = try primaryInfinityInfoFor(ConfigHandle, ops);
            self.infinity_operator_index = @intCast(info.index);
            self.infinity_primary_power = primaryInsertionPower(ops.operators[info.index], info.primary);
            self.infinity_branch_power = self.infinity_primary_power;
        ...
        fn pushPrimaryInfinityPairTerm(self: *Self, config_ptr: *const CorrelatorConfigData, ops: kernel.Call.MultiOp, left_index: usize, right_index: usize, payload: u64) !void {
            ...
            try emitPrimaryInfinityWickTerm(pair, left_index, right_index, wickPayloadTermIndex(payload), self);
        }
        ...
        fn pushCachedPrimaryInfinityPairTerm(branch: *Self, config_ptr: *const CorrelatorConfigData, ops: kernel.Call.MultiOp, left_index: usize, right_index: usize, payload: u64) !void {
            ...
            try emitPrimaryInfinityWickTerm(pair, left_index, right_index, wickPayloadTermIndex(payload), &cache_sink);
        ...
        pub fn pushPairTerm(branch: *Self, config_ptr: *const CorrelatorConfigData, ops: kernel.Call.MultiOp, left_index: usize, right_index: usize, payload: u64) !void {
            if (comptime mode == .primary_infinity) {
...
fn RuntimeWickTactic(comptime ConfigHandle: type, comptime mode: WickMode) type {
    return struct {
        ...
        fn primaryInfinityPayload(payload: u64, ops: kernel.Call.MultiOp, left_index: usize, right_index: usize) u64 {
            if (comptime mode == .primary_infinity) {
                if (opHasInfinity(ops.operators[left_index]) or opHasInfinity(ops.operators[right_index])) {
...
const PrimaryInfinityInfo = struct {
    index: usize,
    primary: Spec.InfinityPrimary,
};
...
fn primaryInfinityInfoFor(comptime ConfigHandle: type, ops: kernel.Call.MultiOp) !PrimaryInfinityInfo {
    var infinity_index: usize = 0;
    var count: usize = 0;
    for (ops.operators, 0..) |op, index| {
        if (!opHasInfinity(op)) continue;
        infinity_index = index;
        count += 1;
    }
    if (count != 1) return error.UnsupportedMultipleInfinityInsertions;

    const infinity_kind = ops.operators[infinity_index].kind;
    for (ConfigHandle.impl.infinity_data) |item| {
        if (item.kind != infinity_kind) continue;
        return switch (item.behavior) {
            .primary => |primary| if (infinityInsertionHasDerivatives(ops.operators[infinity_index]))
                error.UnsupportedInfinityOperator
            else
                .{ .index = infinity_index, .primary = primary },
            .free_boson_exponential => error.UnsupportedInfinityOperator,
        };
    }
...
fn primaryEffectiveWickAvailable(comptime ConfigHandle: type, ops: kernel.Call.MultiOp) bool {
    ...
    const info = primaryInfinityInfoFor(ConfigHandle, ops) catch return false;
    const infinity_kind = ops.operators[info.index].kind;
    // If zero modes can consume the infinity field, the limit adapter must see the base case.
    return !generatedZeroModeCanConsumeKind(ConfigHandle, infinity_kind);
}

fn primaryPointPower(point: operators.Point, power: i16) i32 {
    return if (point.isInfinity()) power else 0;
}

fn primaryInsertionPower(op: kernel.Call.LocalOp, primary: Spec.InfinityPrimary) i32 {
    return switch (op.insertion) {
        .single => |single| if (single.position.isInfinity())
            ...
            0,
        .pair => |pair| primaryPointPower(pair.holomorphic_position, primary.holomorphic_power) + primaryPointPower(pair.antiholomorphic_position, primary.antiholomorphic_power),
    };
}
...
fn InfinityLimitSink(comptime ConfigHandle: type, comptime Sink: type) type {
    return struct {
        ...
        fn ensureInitialized(self: *@This()) !void {
            ...
            const data = self.infinityBehavior(infinity_op.kind) orelse return error.UnsupportedInfinityOperator;
            switch (data.behavior) {
                .primary => |primary| {
                    if (infinityInsertionHasDerivatives(infinity_op)) return error.UnsupportedInfinityOperator;
                    self.infinity_kind = .primary;
                    self.primary_power = singlePrimaryPower(infinity_op, primary);
        ...
        fn emitLimitedBcTopForm(self: *@This(), item: anytype) !void {
            ...
            self.branch_power = 0;
            try self.appendTailCoordinate(coefficient.CoordinateFactor{
                .scalar = coefficient.integer(1),
                .kernel = .{ .difference_power = .{
                    .coordinate = .{ .left = finite[0].raw, .right = finite[1].raw },
                    .exponent = 1,
                } },
            });
        }
