...
pub const Local = opaque {
    ...
    fn nextNormalOrderGroup(self: *Local) !u16 {
        const state = self.data();
        if (state.next_normal_order_group == 0) return error.TooManyNormalOrderedProducts;
        const value = state.next_normal_order_group;
        state.next_normal_order_group +%= 1;
        return value;
    }

    fn withNormalOrderGroup(item: kernel.Call.LocalOp, group: u16) kernel.Call.LocalOp {
        var tagged = item;
    ...
    fn addNormalOrdered(self: *Local, items: anytype) !void {
        const group = try self.nextNormalOrderGroup();
        inline for (items) |item| {
            try self.addRaw(withNormalOrderGroup(try self.rawOperator(item), group));
        }
    }
