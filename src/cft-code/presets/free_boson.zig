...
const pi_atom = scalars.atom(namespace, "pi");
...
fn projectorRank(comptime projector: Handle.TensorProjector) u16 {
    return @intCast(@intFromEnum(projector) >> 24);
}

fn powerOfTwo(comptime power: u16) i64 {
    if (power >= @bitSizeOf(i64) - 1) @compileError("free-boson zero-mode normalization exceeds scalar integer range");
    return @as(i64, 1) << @intCast(power);
}

fn volumeScalar(comptime rank: u16) RuleScalar {
    if (rank > std.math.maxInt(i8)) @compileError("free-boson zero-mode normalization exceeds scalar atom-power range");
    return scalars.monomial(powerOfTwo(rank), 1, 0, pi_atom, @intCast(rank));
}
...
const FreeBosonConfig = struct {
    dimension: u16,
    quantum_schema: []const basis_generation.Quantum = &.{},
};
...
pub fn freeBoson(comptime cfg: FreeBosonConfig) type {
    if (cfg.dimension == 10) return GeneratedFreeBoson10;
    return struct {
...
const GeneratedFreeBoson10 = struct {
    const Base = generated_fixtures.FreeBoson;

    /// op exposes descriptor-generated free-boson local operator builders.
    pub const op = struct {
        /// X builds an explicit bulk free-boson field insertion.
        pub fn X(builder: anytype, mu: anytype, z: Handle.Coord, zbar: Handle.Coord) !Operator {
            comptime shared.assertTargetIndexHandle(@TypeOf(mu));
            return Base.field("X").localPair(builder, z, zbar, .{mu});
        }

        /// dX builds a holomorphic derivative field insertion.
        pub fn dX(builder: anytype, mu: anytype, n: u8, z: Handle.Coord) !Operator {
            comptime shared.assertTargetIndexHandle(@TypeOf(mu));
            return Base.field("dX").localSingle(builder, z, n, .{mu});
        }

        /// dXt builds an antiholomorphic derivative field insertion.
        pub fn dXt(builder: anytype, mu: anytype, n: u8, zbar: Handle.Coord) !Operator {
            comptime shared.assertTargetIndexHandle(@TypeOf(mu));
            return Base.field("dXt").localSingle(builder, zbar, n, .{mu});
        }

        /// expX builds a normal-ordered plane-wave insertion.
        pub fn expX(builder: anytype, k: Handle.Momentum, z: Handle.Coord, zbar: Handle.Coord) !Operator {
            return Base.field("expX").localPair(builder, z, zbar, .{k});
        }

        /// profile builds a target-space profile insertion.
        pub fn profile(builder: anytype, f: Handle.Profile, z: Handle.Coord, zbar: Handle.Coord) !Operator {
            return Base.field("profile").localPair(builder, z, zbar, .{f});
        }

        /// profileVector builds a vector-valued target-space profile insertion.
        pub fn profileVector(builder: anytype, f: Handle.Profile, nu: anytype, z: Handle.Coord, zbar: Handle.Coord) !Operator {
            comptime shared.assertTargetIndexHandle(@TypeOf(nu));
            return declare.operatorBuilder(profile_vector_operator).pair(builder, z, zbar, .{ f, nu });
        }
    };
    /// profile exposes profile-presentation builders.
    pub const profile = ProfileOp;
    /// target exposes deterministic target-space tokens for preset data.
    pub const target = TargetSpace;
    /// config exposes descriptor-generated sphere correlator configs.
    pub const config = Base.config;
    /// basis streams descriptor-generated compact free-boson oscillator words.
    ...
    /// correlator streams descriptor-generated rule matches for free-boson insertions.
    pub fn correlator(config_ptr: anytype, ops: anytype, sink: anytype) !void {
        return shared.streamCorrelator(config_ptr, ops, sink);
    }
};
...
fn sphereZeroStorage(comptime cfg: FreeBosonConfig) [1]zero_mode.Rule {
    const spec = sphereZeroSpec(volumeScalar(cfg.dimension));
...
const sphere_zero_spec = sphereZeroSpec(volumeScalar(10));
...
fn diskZeroSpec(comptime normalization: RuleScalar, comptime volume: RuleScalar) [1]Spec.ZeroModeRule {
    return [_]Spec.ZeroModeRule{
...
fn diskZeroStorage(comptime cfg: FreeBosonBoundaryConfig) [1]zero_mode.Rule {
    const spec = diskZeroSpec(scalars.atomScalar(k_disk_atom), volumeScalar(projectorRank(cfg.neumann)));
    return Spec.zeroModeRules(&spec);
}
...
test "generated free-boson basis matches handwritten dimension-ten basis" {
    const Generated = GeneratedFreeBoson10.basis;
    const Legacy = FreeBosonBasis(.{ .dimension = 10 });

    const Runner = struct {
        fn run(comptime Basis: type) !BasisDigest {
            var sink = BasisDigest{};
            try Basis.stream(3, 3, .{
                .weight = .{ .exact = 3 },
                .max_word_length = 3,
            }, &sink);
            return sink;
